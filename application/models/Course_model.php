<?php
	class Course_model extends CI_Model
	{
		...
		{
			$user = count($this->fetch_model->show(array('CS' =>array('ID'=>$id))));
			if(is_null($id))
			{
				return TRUE;
			}
			...
			{
				$this->errorlog_library->entry('Course_model > check > argument ID is invalid.');
				redirect('Course/add/');
			}
		}
		...
		public function add_or_edit()
		{
			...
			{
				unset($_POST['ID']);
				$result = $this->form_model->add(array("table"=>"CS","columns"=>$_POST));
			}
			else
			{
				$result = $this->form_model->edit(array("table"=>"CS","columns"=>$_POST,"where"=>array('ID'=>$_POST['ID'])));
			}				
			if($result === TRUE)
			{
				echo 1;
