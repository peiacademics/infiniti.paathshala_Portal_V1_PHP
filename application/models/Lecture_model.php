<?php
	class Lecture_model extends CI_Model
	{
		...
		{
			$this->load->model('form_model');
			$_POST['class_ID'] = $_POST['ID'];
			$date = str_replace('|', '-', $_POST['date']);
			$_POST['date'] = date('Y-m-d', strtotime($date));
			unset($_POST['ID']);
			if($_POST['lect_ID'] == NULL)
			{
				unset($_POST['lect_ID']);
				$result['stat'] = $this->form_model->add(array("table"=>"LEC","columns"=>$_POST));
				$result['id'] = $_POST['class_ID'];
				$result['lect_ID'] = $this->db_library->find_max_id('LEC');;
			}
			else
			{
				$id = $_POST['lect_ID'];
				unset($_POST['lect_ID']);
				$result['stat'] = $this->form_model->edit(array("table"=>"LEC","columns"=>$_POST,"where"=>array('ID'=>$id)));
				$result['id'] = $_POST['class_ID'];
				$result['lect_ID'] = $id;
			}
			return $result;
			...
			if ($data) {
				foreach ($data as $key1 => $value1) {
					...
					if(($lectures != FALSE) && !empty($lectures) && ($lectures != NULL))
					{
						...
						$data[$key1]['chapter_name'] = $this->str_function_library->call('fr>LS>name:ID=`'.$lectures[0]['chapter'].'`');
