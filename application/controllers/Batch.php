<?php
	...
	{
		$this->data['breadcrumb']['heading'] = 'Batch';  
		$this->data['breadcrumb']['route'] = array(array('title'=>'Dashboard','path'=>'dashboard'),array('title'=>'Settings','path'=>'settings'),'Batch');
	...
	public function get_show_data($item_id = NULL)
	{
        ...
        {
        	$res = $this->batch_model->get_show_data('BT',array('name','description','>>branch:>fr>BR>name:ID=^branch_ID^','ID'));
        }
        else
        {
        	$branch = $this->session_library->get_session_data('branch_ID');
        	$res = $this->batch_model->get_show_data(array('BT'=>array('branch_ID'=>$branch)),array('name','description','ID'));
        }
 	...
 	public function add($id=NULL)
 	{
		...
		$this->data['breadcrumb']['route'] = array(array('title'=>'Dashboard','path'=>'dashboard'),array('title'=>'Settings','path'=>'settings'),array('title'=>'Batch','path'=>'Batch'),'Add');
		...
		else
		{
			if($check)
			{
				if(!is_null($id))
				{
					...
					$this->data['breadcrumb']['route'] = array(array('title'=>'Dashboard','path'=>'dashboard'),array('title'=>'Settings','path'=>'settings'),array('title'=>'Batch','path'=>'Batch'),'Edit');  
					$this->data['What'] = 'Edit';
 	...
 	public function show($id)
 	{
 		if (!empty($id))
 		{
 			...
 			$this->data['breadcrumb']['route'] = array(array('title'=>'Class','path'=>'batch/show/'.$id),'Show');
	 		$this->load->view('includes/header',$this->data);
			$this->load->view('pages/batch_show',$this->data);
			$this->load->view('includes/footer',$this->data);
 		}
 		else
 		{
 	...
 	{
 		echo json_encode($this->batch_model->addClass());
 	}

 	public function classes($branch_ID = NULL)
 	{
 		echo json_encode($this->batch_model->classes($branch_ID));
 	}

 	public function get_all_students($branch_ID = NULL)
 	{
 		echo json_encode($this->batch_model->get_all_students($branch_ID));
 	}
 	// public function getPersons()
 	// {
 	// 	echo json_encode($this->batch_model->getPersons());
 	// }
 	...
 	// {
 	// 	echo json_encode($this->batch_model->addTimeline());
 	// }
}
?>