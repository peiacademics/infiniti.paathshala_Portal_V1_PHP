...
class SSP {
	 ...
	 */
	static function limit ($start,$length,$data,$request)
	{
	 ...
	 */
	public function simple ( $request, $conn, $table, $primaryKey, $columns,$input,$output,$options=null)
	{
		// print_r($request);
		// print_r($request);
		// var_dump($options);
		if (empty($request['search']['value'])) {
			$options = array('LIMITS'=>array($request['length'],$request['start']));
		}
		
		// $options = array('LIMITS'=>array(10,10));
		$data = $this->ci->fetch_model->show($input,$output,$options);
		// print_r($data);
		 				...
		 				else
		 				{
					        ...
					        $(document).ready(function() {
		 					...
		 					else
		 					{
		 						...
		 						else if ($action['icon']=="external-link") {
		 							// $data[$key]['Link'] .= "<span class='label label-info ".$action['class']."' id='item".$value['ID']."' onClick='view(\"".$value['ID']."\",\"".base_url($action['link'].$value['ID'])."\")'><i class='fa fa-".$action['icon']." bigger-130'></i></span>&nbsp;&nbsp;";
		 ...
		 */
		if (empty($request['search']['value'])) {
			$count = $this->ci->fetch_model->show($input,'count(ID)');
		    $recordsTotal = ($data) ? $count[0]["count(ID)"] : 0;
		}
		else
		{
			$count = $this->ci->fetch_model->show($input,'count(ID)');
			$recordsTotal = ($data) ? $count[0]["count(ID)"] : 0;
			$count[0]['count(ID)']=count($data);
			// echo $count[0]['count(ID)'];
			// echo "string";
		}

$data = self::limit($request['start'],$request['length'],$data,$request);
		$recordsFiltered = ($data) ? $count[0]["count(ID)"] : 0;
		return array(
