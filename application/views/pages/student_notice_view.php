<div class="page-content">
  <div class="<?php echo ($this->data['Login']['Login_as'] == 'DSSK10000011' || $this->data['Login']['Login_as'] == 'DSSK10000012') ? 'hidden' : ''; ?>">
    <div class="pull-right">
       <a href="<?php echo base_url('student_notice/add/'.$branch_ID); ?>"><button typer="button" class="btn btn-w-m btn-primary dim btn-outline"><i class="fa fa-plus"></i> Add Student Notice</button></a>
    </div>
  </div>
  <div class="row">
    <div class="ibox">
      <!-- <div class="ibox-title">
        <h5><?php echo ucfirst($this->lang_library->translate('Student Notices')); ?></h5>
...
<!--           <a href="<?php echo base_url('student_notice/add/'.$branch_ID); ?>" class="btn btn-w-m btn-primary">Add Student Notice</a>
...
<div class="modal inmodal in" id="view_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content animated flipInX">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
        <h4 class="modal-title">Student Notice Record</h4>
      </div>
      <div class="modal-body">
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    oTable = $('#example').DataTable( {
      "processing": true,
      "serverSide": true,
      "ajax": "<?php echo base_url('student_notice/get_show_data'); ?>"+'/'+branch_ID,
      dom: 'lBfrtip',
        buttons: [
...
function student_attendace(id)
{
  ...
  $.ajax({
    type:'POST',
    url: '<?php echo base_url(); ?>'+'student_notice/student_attendace/'+id,
    success:function(response)
    {
...
function save_attendance()
{
  ...
  $.ajax({
    ...
    data:fdata,
    dataType:'json',
    url: '<?php echo base_url(); ?>'+'student_notice/save_attendance',
    success:function(response)
    {
...
function view(id)
{
  ...
  $.ajax({
    type:'POST',
    url: '<?php echo base_url(); ?>'+'student_notice/view/'+id,
    success:function(response)
    {
      ...
      data += '<tfooter><tr><td colspan="2" class="text-center"><button type="button" id="cls" class="btn btn-lg btn-danger" data-dismiss="modal">Close</button> <button id="cmt_add" type="button" class="btn btn-lg btn-success" onClick="commun_add(\''+id+'\');">Communicate Student Notice</button></td></tr></tfooter></table>';
...
function commun_add(id)
{
  $.ajax({
    ...
    method:'POST',
    data:{ID:id,rec_id:'CMSSK10000006',tbl:'SN'},
    datatype:'JSON',
    success:function(response){
      response = JSON.parse(response);
