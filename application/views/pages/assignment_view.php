...
function student_attendace(id)
{
  ...
  $.ajax({
    ...
    success:function(response)
    {
      var data = '<table class="table table-striped table-bordered" width="100%"><thead><tr><th class="text-center">Sr. No.</th><th class="text-center">Name</th><th class="text-center">Status</th><th class="text-center">Latency Reason</th><th class="text-center">Remark</th></tr></thead><tbody>';
      if(response != '')
      {
        response = JSON.parse(response);
        var i = 1;
...
function view(id)
{
  ...
  $.ajax({
    ...
    success:function(response)
    {
      ...
      data += '<tfooter><tr><td colspan="2" class="text-center"><button type="button" id="cls" class="btn btn-lg btn-danger" data-dismiss="modal">Close</button> <button id="cmt_add" type="button" class="btn btn-lg btn-success" onClick="commun_add(\''+id+'\')">Communicate Assignment Generation Details</button></td></tr></tfooter></table>';
...
function commun(id)
{
  $.ajax({
    ...
    method:'POST',
    data:{ID:id,rec_id:'CMSSK10000011',tbl:'AS'},
    datatype:'JSON',
    success:function(response){
      response = JSON.parse(response);
...
function commun_add(id)
{
  $.ajax({
    url:'<?php echo base_url("communicate/get_record"); ?>',
    method:'POST',
    data:{ID:id,rec_id:'CMSSK10000001',tbl:'AS'},
    datatype:'JSON',
    success:function(response){
      response = JSON.parse(response);
      $('#branch_added select option[value="'+response.rec.branch_ID+'"]').prop('selected', true).trigger("chosen:updated");
