<div class="page-content">
  <div class="pull-right">
     <a href="<?php echo base_url('syllabus_coverage/add/'.$branch_ID); ?>"><button typer="button" class="btn btn-w-m btn-primary dim btn-outline"><i class="fa fa-plus"></i> Add Syllabus Coverage</button></a>
  </div>
  <div class="row">
    <div class="ibox">
      ...
      <div class="ibox-content table-responsive">
        ...
        <div class="tabs-container">
          ...
          <?php foreach ($subjects as $key => $value) { ?>
            <li id="list-<?php echo $key; ?>" class="active"><a data-toggle="tab" href="#tab-<?php echo $key; ?>" aria-expanded="true"><i class="fa fa-trophy"></i> <?php echo $value['name']; ?></a></li>
           <?php } ?>
          </ul>
          <div class="tab-content">
            ...
            <div id="tab-<?php echo $key; ?>" class="tab-pane active">
              <div class="panel-body">
                <table id="example-<?php echo $key; ?>" class="display responsive nowrap" cellspacing="0" width="100%">
                  <thead>
                    <tr>
                      <th>Batch</th>
                      <th>Chapter</th>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    <?php foreach ($subjects as $key => $value) {?>
    oTable<?php echo $key; ?> = $('#example-<?php echo $key; ?>').DataTable( {
      "processing": true,
      "serverSide": true,
      "ajax": "<?php echo base_url('syllabus_coverage/get_show_data'); ?>"+'/'+branch_ID+'/<?php echo $value['ID']; ?>',
      dom: 'lBfrtip',
      "buttons": [
    ...
    for(var i = 0; i < <?php echo count($subjects); ?>; i++)
    {
      if(i != 2)
      {
      ...
      if(response != '')
      {
        ...
        if(response.present != null)
        {
          $.each(response.present, function(key,value){
            data += '<div class="feed-element"><a href="'+base_url+'syllabus_coverage/show/'+value.ID+'" class="pull-left"><img alt="image" class="img-circle" src="'+base_url+value.path+'"></a><div class="media-body "><strong>'+value.name+'</strong></div></div>';
          });
        }
        else
        {
