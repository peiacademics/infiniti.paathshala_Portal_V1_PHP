      ...
      <div id="data_table" class="">
        ...
        if (!empty($lists['recs'])) {
          ...
          <tr>
            ...
            <div class="col-sm-3">
              <span><?php echo $value['lead_reason'] ?></span>
            </div>
            <!-- </td>
            <td> -->
            <div class="col-sm-4">
              <div class="smlBtn visible-sm visible-xs">
                  <div id="time<?php echo $value['ID']; ?>"></div>
                    <?php if ($value['lead_calls']['call_Status']==='recall') {
                 ...
                 <?php    } ?>
              <?php if ($value['lead_calls']['call_Status']==='blank') { ?>
              ...
              <?php }else if($value['lead_calls']['call_Status']==='called'){ ?>
              <div id="<?php echo $value['ID'];?>">
              ...
              <?php }else if($value['lead_calls']['call_Status']==='accept'){ ?>
              <div id="<?php echo $value['ID'];?>">
              ...
              <?php } else if($value['lead_calls']['call_Status']==='customer') { ?>
                <div id="<?php echo $value['ID'];?>">
                  <a class="btn btn-success btn-block disabled"> <i class="fa fa-tick"></i> Added in Customer</a>
                </div>
              <?php }else if($value['lead_calls']['call_Status']==='recall'){ ?>
                <div id="<?php echo $value['ID'];?>">
...
function addIcons(id,status,id1,k){
  ...
  else
  {
    $.ajax({
      ...
      dataType: "json",
      url: '<?php echo base_url();?>Leads/lead_call_status',
      success:function(response)
      {
       // response=JSON.parse(response);
        if (status==='called'){
...
$('#addRecall').on('click',function(e) {
  ...
  $.ajax({
    ...
    data:formdata,
     dataType: "json",
    url: '<?php echo base_url(); ?>'+'Leads/recall/',
    success:function(response)
    {
